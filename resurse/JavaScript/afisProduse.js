	...
	*/
	...
	function afiseajaJsonTemplate(obJson) { 
			...
			for(let i=0;i<obJson.produse.length;i++){
				...
				textTemplate+=ejs.render("<div class='templ_produse'>\
				<p>Tip: <%= produs.tip %></p>\
				<p>Nume: <%= produs.nume %></p>\
				<p>Descriere: <%= produs.descriere %></p>\
				<p>Gramaj: <%= produs.gramaj %></p>\
				<p>Alergeni: <%= produs.alergeni %></p>\
				<p>Pret: <%= produs.pret + ' RON' %></p>\
				<p>Bucati: <%= produs.bucati %></p>\
				</div>",
